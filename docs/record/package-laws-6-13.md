...
| 9 | Import resolves existing artifacts and pins without running setup or fetching. Missing requirements/artifacts name `setup!`. Explicit setup may use the existing pinned Git importer. Native loading takes the engine's one door, `metta_load_source/2`, since 0a81c782f: a half inside the boot's stamped set (`lib/*/*.pl`) may start the boot's hermetic compile child, which fetches nothing and builds no requirement, only the engine's own `.qlf` cache of a source already present, and falls back to an in-process load when no child can run; a package outside the stamped set, a git-fetched checkout or a program's own directory, starts no child. | `import_never_runs_setup`, `missing_artifact_names_setup_as_the_remedy`, `offline_git_requires_a_lock`, `setup_pins_git_and_a_fresh_import_spawns_nothing`. The last case prepares a local Git repository, then imports in a fresh process with process creation disabled; that process loads `engine/metta.pl` without `engine/qlf_boot.pl`, so it holds no claim and cannot reach the compile child either way. The claim's two sides are `a_backing_row_loads_its_half_through_the_claim` (a shipped half reads its artifact) and `an_unclaimed_backing_loads_from_source_and_leaves_no_artifact` (a package outside the stamped set compiles from source and gains nothing), each with its disabling mutation, and `extensions/python/tests/repository/test_library_halves.py`, which counts the children: one per claimed half in the first process, none in any process after. |
