...
### The classes branch re-takes its pins
Tip d761b9c43 (functional a8cfae1f5), the patched SWI-Prolog 10.1.14 host, every measurement in a provisioned battery worktree with the .qlf set rebuilt. The counter rows of `baseline.json` (31 moved beyond the four-inference allowance) are re-pinned row-locally with `ai_repin_baseline.py`, each comment naming the steps the first-parent ladder of c66cf0c10..2f1be9e0b, its refined brackets and the classes window placed (`ai-pin-ladder.json` and the three `ai-window-ladder.json` files under the batteries' `ai-tmp`, merged by `ai_counter_moves.py`); typed-call's slope pin moves with the same two inferences a typed call lost at 6c7094699. The automatic-tabling growth pins in `test_benchmarks.py` read the branch's constant (+38 plain) and the tabling unit's per-cached-call node and verdict (+685 to +1005 automatic). The cost rows and the scaling families are re-recorded with `--record` on the tip and composed by `ai_compose_ledger.py`, which keeps each row's chain and appends one RE-PINNED line: the cost rows carry the branch's per-evaluation constant (+12 at e67e2db94, +6 at 9c6e52599, +14 in 039974720..317fcdd6a, +3 in 317fcdd6a..6444e7673, -3 at ed3cef513, +5 in 7942599d2..e7ba2730f, +37 in 0312c074f..f77b3d48f, -6 in 0bd522b50..7ead07e09, +3 in 45fb2d088..b9ebafac7, -32 at d60277df1), subtraction's -7.2 percent in c66cf0c10..7942599d2 and the quadratic controls' first-size +866 in 7079813e6..3c9aec392; the scaling families' moves all predate the ladder (chain-join -80 percent, segment-split to selective-query's constant, parse-forms -20 percent, all in the 2026-08-31 and 2026-09-02 brackets of eight commits, not split further), and segment-split's class is no longer observed by the inference counter, which the row now says.
Measured: the pre-ladder sweeps under `PeTTb/ai-tmp/ai-sweep-scaling-pre2-*.json` (103 points of 0a0d6d4a1..c66cf0c10) and `wt-battery-6/ai-tmp/ai-sweep-costs-*.json` (the 27 ladder points and 13 pre-ladder points), the refinements stopped at the user's word once the placements above were in hand.
Open: the memory ledger, the extension-cost rows, the instructions lane, the engine bench, the seat benches and boot determinism are re-taken on a quiet box after the gate; the twins are re-pinned corpus-wide with `twin_coverage.py --repin` on the tip.
