...
### T3: declared types at the owned doors

Tried: the downstream consumer `tests/typing/audit_surface.py` reports seven mypy diagnostics on the tip before annotation (`server.host` read as `str | bytearray`, gateway replies as `dict[Any, Any]`, capsule doors as `Any`, `answers[0]` as `int` only through an overload that put SupportsIndex before Variable, and two negative controls no longer refused); `ai-tmp/ai-audits-t3-consumer-baseline.log`. ty on the tip reports four diagnostics that were absent at the cut (`ai-tmp/ai-audits-control-ty.log`, green): two after c07bb08a0 and 7491c22b7 (`__required_keys__` read on `type`; a `functools.partial` inserted into a `list[Callable[[], None]]`), two after 96b907668 (`Answers(records)` inferred as `Answers[_AnswerItem[T] | T]` in `_slice`); `ai-tmp/ai-audits-pre-f14-ty.log`, `ai-tmp/ai-audits-f14-ty.log`.

Decided: `Point.__init__` declares its ten attributes and the twelve module points are `Point`; `_Inverse` actions are `Callable[[], object]` because a reconciliation partial returns the listener's own result; `_registration_compensator` owns the listener-result contract so `_enlist` raises nothing inline. `_into_fields` returns one `_Into` record (named inputs, required names, the constructor call) so `rows_into` neither re-derives required keys nor branches on a None signature; the TypedDict split is read from the class namespace the metaclass writes. `Answers.__hash__` is `ClassVar[None] = None` with typeshed's own assignment ignore, since None is the data model's spelling and a raising method still advertises Hashable. `Answers._from_records` builds the two `_slice` views: ty 0.0.71 collects every union member's constraint for `Iterable[T | _AnswerItem[T]]`, so a record source widens the value kind, and `bound_source` is typed over records because its one provider (`_spaces/query.py:bounded_engine_source`) and its one argument (`_items()`) are records. Gateway and serve take `m: object`, the tolerant `space_of` contract; wire envelopes are `dict[str, object]`; `ssl_context` is `SSLContext | None`; `TableBridge._projection` returns `Projection` from a module-level import and the capsule doors return `object`; the remote package annotates its PEP 562 hooks; the eleven algebra presets are `DeclaredAlgebra`.

Rejected: `Answers[T](...)`, which ty still widens and which adds a `types.GenericAlias` call per slice (`ai-tmp/ai-audits-ty-probe/probe.py`); `__init__` overloads over records and values, which mypy refuses for a mixed source (`probe2.py`); `logger.exception` at the three remote boundaries, a WARNING-to-ERROR change no rule required, since the committed spelling passes the seat's Ruff lane (`ruff check --stdin-filename metta/remote/_gateway.py`).

Measured: `PYTHONPATH=extensions/python basedpyright --pythonpath /home/user/Dev/.venv-pypetta/bin/python --verifytypes metta --ignoreexternal --pythonversion 3.14 --outputjson` (basedpyright 1.39.6) reports completeness 0.7381562099871959 with 1153 known, 27 ambiguous and 382 unknown exported symbols, against 0.7098844672657253 (1106, 65, 387) on the unchanged cut; both exit 1 for incomplete types. `ai-tmp/ai-audits-t3-final-verifytypes.json`. The 409 remaining unknown or ambiguous exports by module file: `metta/__init__.py` 83 (generated root), `vocabularies.py` 50 (classes-held, generated), doors' files 80, classes' files 75 (`parallel.py` 16, `library/__init__.py` 9), other unassigned modules 69 (`testing/__init__.py` 34, `foreign/__init__.py` 32), audits' files 52 (`algebra/__init__.py` 41, `tables.py` 7, `remote/__init__.py` 4). Every audits-file entry carries no direct missing-annotation diagnostic; each inherits a partially unknown root: `metta.Atom`, `metta.Expression` (`_atoms/model.py`), `metta.Space`, `metta.MeTTa` (`_faces/space.py`), `metta.MettaError` (`_errors/errors.py`), all classes-held, and `SemiringOrder` from `vocabularies.py`. The audit's 63.96% was measured on the 2026-09-01 tree and is not comparable to either number.

Verified: mypy, mypy-root-impl, the three typing fixtures, the template fixture and the new consumer pass (`ai-tmp/ai-audits-t3-mypy-final.log`); ty passes (`ai-tmp/ai-audits-t3-ty-2.log`); Ruff passes over the seat; stubtest, init-stub, init-stub-selftest and closed-sets pass (`ai-tmp/ai-audits-t3-lanes.log`); the focused battery reports 143 passed (`ai-tmp/ai-audits-t3-focused.log`) and the spaces, extension, concurrency, getting-started and remote chapters report 1412 passed and 2 skipped in 132.49 seconds (`ai-tmp/ai-audits-t3-chapters.log`); `tools/reference.py --write` rewrote the remote, results, tables and root pages (`ai-tmp/ai-audits-t3-regenerate.log`).
...
Decided: the stranger declares `rows=frame_rows`, the way both shipped frame providers do, and the refusal names that declaration so the next library reads the remedy rather than inferring it. The two provider tests register a counting replacement row through `seam.frame.register` and assert ingestion called it, which proves the declared path was taken rather than that one other path was not, and needs no private name. The graphql test registers its watching row the same way. The four suppressions go at the cause: `refuse(RefusalKind.capability, ...)` is the typed vocabulary member rather than a string literal (EM101), `_batches_of(data, acquire)` is the extraction dispatch TRY301 asked for, and the two exit tests raise through a local `fail()`.

Verified: stranger-python, layering, layering-selftest, llms, llms-selftest, binding-selftest, no-hardcoded-integration and its selftest, extension-scaffold, reference and reference-selftest pass; ruff passes over the seat; 152 gate-completeness/exit/dataset cases and all 231 ext cases pass; `ai-tmp/ai-audits-stranger-python.log`, `ai-tmp/ai-audits-layering-lane.log`, `ai-tmp/ai-audits-llms-lane.log`, `ai-tmp/ai-audits-burndown.log`, `ai-tmp/ai-audits-ext-suites.log`.

Tried: four more of the gate's reds were the isolated worktree, not the tree. `extensions/node/node_modules`, `extensions/node/browser`, `extensions/node/_runtime` and `website/node_modules` are build output that `worktree.sh` does not copy, so `binding-selftest`'s node leg could not import esbuild, the site-build refusal test got a module-not-found instead of the refusal, `llms` read five node paths as absent, and `node-bench` reported `the Node benchmarks will not run without swipl-wasm` and passed without measuring anything. Linking the two node_modules and copying the two built directories from the main checkout turns all four green (`ai-tmp/ai-audits-node-provisioned-lanes.log`, `ai-tmp/ai-audits-node-web-tests.log`); all four stay untracked.

### The instructions lane's save-load pins move with the seat's code size, not its work

Tried: the whole gate's `instructions` lane reports `save-load-fast` and `save-load-metta` regressing on this branch (+5.3% and +8.0%) where the baseline reported `py-method-call` instead. Interleaved measurement, same box, same TMPDIR, min-of-three per run: the cut reads 5.125/5.123 and 3.481/3.481 (G instructions), this branch 5.409/5.420 and 3.770/3.770. `ai-tmp/ai-audits-instr-branch-*.log`, `ai-tmp/ai-audits-instr-cut-*.log`.

Tried: bisecting in a read-only control worktree at each commit, compiled artifacts purged and warmed between measurements (`ai-tmp/ai-audits-instr-probe.sh`). save-load-metta: efac42a31 3.479, F15 3.479, F07 3.483, F05 3.451, F11 3.756, F04 3.757, F14 3.759, tip 3.758. The step is F11 (c07bb08a0), whose only runtime change is `_into_fields` and `rows_into`.

Tried: isolating inside that file. The `Rows.into` docstring alone, the module header alone, the new `_into_fields` alone and the new `rows_into` alone each read 3.44-3.46; the two functions together read 3.756. A probe printing a stack trace inside `_into_fields` shows the benchmark never calls it. The round trip's own counters are identical either way: 1,046,550 inferences, 20,001 atoms saved, SWI `global_shifts` 9, `local_shifts` 8, `trail_shifts` 7, `globalused` 18,322,576, and the operation runs no slower by wall clock (0.309 s against 0.384 s). Outside the harness the whole command reads 39.132/39.161 G instructions, a 0.075% difference.

Tried: a `_audit_layout_probe` of the same size as F11's change, never called, planted on the unchanged cut. It reproduces the step exactly: 3.755-3.760. Thirty-two lines of comments do not (3.450), and padding `PATH` by up to 256 characters does not (3.479-3.484), so it is the module's bytecode size rather than the source or the environment block.

Measured: `gc.collect(); gc.freeze(); gc.disable()` immediately before the measured window removes the effect. With the layout probe planted and the collector frozen the case reads 3.450-3.464, the same as the cut without the probe (3.451-3.465). So a cyclic collection falls inside the controlled window in one mode and outside it in the other, and an unrelated allocation-count shift anywhere in the seat decides which.

Decided: record it, do not re-pin. The pins are the integrator's, and the remedy is theirs to choose: `extensions/python/benchmarks/pure.py` can freeze and disable the collector around the measured operation, which is `timeit`'s own reason for doing so ("it makes independent timings more comparable", https://docs.python.org/3/library/timeit.html), consistent with this harness already pinning `PYTHONHASHSEED`, the environment block and ASLR for reproducibility. pyperf and pyperformance deliberately do the opposite and leave GC, hash seed and ASLR alone, because they measure production-representative behaviour rather than a fixed configuration (https://pyperformance.readthedocs.io/usage.html, "Don't disable GC nor ASLR"); this harness has already chosen the other corner, and the collector is the one source of mode-flipping it has not pinned. Frozen, the case reads about 1% BELOW its current pin, so the change needs one re-pin of the affected rows.

Open: `save-load-fast`, `save-load-metta` and `py-method-call` stay red on the `instructions` lane, with no work added by this branch: identical inferences, identical stack shifts, and the step reproducible from a never-executed block of the same size.

### The Node seat's capability roster was a second copy of the row

Tried: the final whole gate's `node-binding` lane failed one of 650 cases, `names every capability the engine's own row carries`: the booted catalog carries `savepoint` and `extensions/node/src/provider.ts:CAPABILITIES` does not. The F12 vocabulary word reached the generated `extensions/node/src/vocabularies.ts` when vocabgen ran, but the seat's own roster is written out by hand beside it. The lane could not have caught it earlier: without `extensions/node/node_modules` it exits 0 with `note: run 'npm ci --prefix extensions/node'`.
