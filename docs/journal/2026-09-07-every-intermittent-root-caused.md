...
## 2026-09-11, why SWI declined that one resolution
Tried: the final gate of the end-of-wave battery on b1d175f13 -> seven failures in `tests/ch18_performance/test_materialization.py`, every one `ugraphs:vertices_edges_to_ugraph/3: Unknown procedure: ugraphs:append/3`, items 713 to 719 of one xdist worker, the item before the first being `test_a_bounded_first_load_rolls_back_materialization_preprocessing`, an inference-limited load. Replayed alone under the gate's seed (`-p randomly --randomly-seed=571967428`) -> 0 failures. The same ball as the `prolog_wrap:member/2` section above, after an inference-limited derivation, whose cause that section left open.
Measured: bare SWI 10.1.13, `swipl -q -f none -g "use_module(library(ugraphs)), catch(call_with_inference_limit(vertices_edges_to_ugraph([a],[a-b],_),B,_),_,true), vertices_edges_to_ugraph([a],[a-b],G)" -t halt` -> `existence_error(procedure, ugraphs:append/3)` at every budget B from 20 to 1,000 and `ok` at a budget the goal does not reach. A fresh module whose clause calls `sum_list/2` and then another goal, one module per budget, 64 budgets -> 64 poisoned. The same clause with the call as its last goal -> 0 poisoned. After the poison: a meta-call of the predicate resolves it and repairs the compiled site, an explicit `import/1` repairs it, a defining assert repairs it; `dynamic/1`, `discontiguous/1` and `abolish/1` do not touch it.
Mechanism, read in the 10.1.13 source: the trap for an undefined predicate (src/pl-proc.c `trapUndefined`) calls `autoLoader`, which runs `system:'$undefined_procedure'/4` as a `PL_Q_PASS_EXCEPTION` query and reads its answer atom. A query that raised leaves the answer at `ATOM_nil`, which is neither `fail`, `error` nor `retry`, so `trapUndefined` falls to its error path and calls `createUndefSupervisor` on the definition (src/pl-supervisor.c `undefSupervisor`, taken when the definition has no clauses). Every later compiled call that is not a last call dispatches straight to that supervisor (src/pl-vmi.c `depart_or_retry_continue` ends in `PC = DEF->codes`, and `S_UNDEF` raises without re-trapping), while a last call, a meta-call, an import (which replaces `proc->definition`) and a first clause (which resets the supervisor) all pass through `getProcDefinedDefinition` or replace the codes. The limit lands inside the resolution whenever the bounded goal is the first to reach the predicate, and the resolution's `absolute_file_name/3` walk is hundreds of inferences wide, which is why an inference-limited test poisons the library predicate for every test after it on the same worker.
Rejected, each by measurement in bare SWI over a 200-budget sweep: a last `user:exception/3` clause that runs `'$autoload'` under a catch, because the trap's query has call ports before any hook clause runs and a trip there poisons with nothing run; re-throwing the ball from inside the trap's query after finishing the resolution, because the C trap then continues into the resolved predicate with the ball pending, the first foreign call reports "did not clear exception" and drops it, and the bounded goal completes as if unbounded (105 of 200 budgets returned `!`); finishing the resolution inside `prolog:prolog_exception_hook/5`, because a foreign predicate returning inside the hook is reported the same way; re-arming the limit to trip again, because SWI's limit stack is popped by its own catcher and an unpaired push corrupts it; a C predicate resetting the supervisor, because `freeCodesDefinition` and the definition struct are internal headers and the rule is no fork and no reach into libswipl; eager imports site by site, the 2026-09-07 decision, because the class stays open and the `no-autoload` lane cannot see an explicit `autoload/2` declaration (under the flag it becomes an immediate import).
Decided: `engine/metta/limits.pl`, a host workaround under the ledger key `swi-autoload-cut-installs-the-undefined-supervisor`, with its reproduction and a plunit suite (`tests/prolog/suites/host/limits.plt`). `'$undefined_procedure'/4` is wrapped with `wrap_predicate/4`: the resolution runs under a catch, a cut resolution is run again once the limit has disarmed (src/pl-prims.c `raiseInferenceLimitException` sets `INFERENCE_NO_LIMIT` before it raises), the second attempt's answer is returned, and the ball is re-raised through `thread_signal/2`, which throws it from the next call port, the first port of the resolved predicate: 200 of 200 budgets callable afterwards, every cut budget reporting `inference_limit_exceeded`, no warnings. A trip on the query's own entry ports, before the wrapper's catch, is repaired from `prolog:prolog_exception_hook/5` by scheduling `'$define_predicate'/1` through `thread_signal/2` for the thread's next safe point (budgets 1 and 2 of the sweep reach it); only the inference limit's ball is repaired there, because reading a frame marks it and a time limit or an interrupt can surface at an engine's outer query frame, which the receipts listener cannot survive (the swi-query-frame-discarded-on-engine-destroy entry). The `prolog_wrap:import(lists:member/2)` line stays as the cheaper resolution of a predicate every boot reaches; its comment now names the mechanism.
Open: the upstream report is the user's to file (a one-line fix in `trapUndefined`: leave the definition untouched when the resolution query raised, so the next call traps again). A time limit or an interrupt landing on the trap query's entry ports is the documented residue.
