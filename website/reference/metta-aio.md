...
Source: `extensions/python/metta/aio/__init__.py`.

<!-- Generated by extensions/python/tools/reference.py from the named source and its imports; lane=reference. -->

> Expose worker-owned asynchronous operations and result views.
>
> caller supplies another bound.
> cancelled startup closes the worker.
...
```python
class AsyncMeTTa(aio._worker.AsyncMeTTaBase):
```
...
```python
async def call(self, fn: Callable[[Space], T]) -> T:
```
...
```python
async def reify(self) -> aio._views.AsyncWorld:
```
...
```python
async def commit(self, world: aio._views.AsyncWorld) -> None:
```
...
```python
def saga(self, receipts: AsyncMeTTaBase) -> aio._views.AsyncSaga:
```
...
```python
def batch(self) -> aio._views._AsyncBatch:
```
...
```python
def stats(self) -> aio._views._AsyncStats:
```
...
```python
def assuming(self, *facts: Any) -> aio._views._AsyncAssuming:
```
...
```python
async def prepare(self, *patterns: Any, where: Any | None = None) -> aio._views._AsyncPrepared:
```
...
```python
def fn(self) -> aio._views._AsyncFunctionNamespace:
```
...
```python
async def aclose(self, timeout: float = aio.DEFAULT_CLOSE_TIMEOUT) -> None:
```
...
```python
def stop(self, timeout: float | None = aio.DEFAULT_CLOSE_TIMEOUT) -> None:
```

> Synchronously cancel streams and stop; None waits until the worker exits.

### `AsyncMeTTa.__aenter__`
...
### `AsyncMeTTa.speculate`

```python
async def speculate(self) -> ScopedExecution:
```
...
```python
async def unregister_token(self, pattern: str | re.Pattern[str]) -> None:
```
...
```python
async def covers(self, effect: vocabularies.EffectClass | str) -> Atom:
```
...
```python
async def add_tagged_rule(self, tag: Any, head: Any, *premises: Any, where: Any = None) -> Atom:
```
...
```python
async def image(self, type_name: str, setting: vocabularies.ImageMode) -> Atom:
```
...
```python
async def consumption(self, kind: vocabularies.SourceKind) -> Atom:
```
...
```python
async def merge(self, pattern: str | Atom, policy: vocabularies.AnswerPolicy) -> Atom:
```
...
```python
async def context(self, world: vocabularies.World) -> Atom:
```
...
```python
async def agenda(self, policy: vocabularies.AgendaPolicy, function: str | None = None) -> Atom:
```
...
```python
async def atomicity(self, atomicity: vocabularies.Atomicity) -> Atom:
```
...
```python
async def emits(self, policy: vocabularies.AnswerPolicy) -> Atom:
```
...
```python
async def effect_plan(self, target: Any) -> EffectPlan:
```
...
```python
async def lint(self) -> list[lint._model.Finding]:
```
...
async def live(
    self,
    *query: Any,
    on: vocabularies.SubscriptionEdge = vocabularies.SubscriptionEdge.both,
