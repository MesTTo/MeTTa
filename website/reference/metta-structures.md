...
### `ClosureView.__init__`

```python
def __init__(self, space: SpaceLike, relation: str, *, symmetric: bool = False) -> None:
```

No docstring is defined.

### `ClosureView.__contains__`
...
```python
def __init__(self, space: Any, pattern: Any) -> None:
```

No docstring is defined.

### `LiveView.__contains__`
...
```python
def __init__(self, space: SpaceLike, name: str, *, arity: int | None = None) -> None:
```

No docstring is defined.

### `TabledMap.__getitem__`
