...
compare atoms; comparison terms name their head explicitly. In the table,
`x1` and `x2` are atoms. Function spellings use `import builtins, math, operator`.

<!-- begin generated atom operators -->
...
| `x1 * x2` | `(* x1 x2)` |
| `x1 / x2` | `(/ x1 x2)` |
| `x1 // x2` | `(floor-math (/ x1 x2))` |
| `x1 % x2` | `(% x1 x2)` |
| `x1 ** x2` | `(pow-math x1 x2)` |
| `x1 @ x2` | `(matmul x1 x2)` |
| `x1 << x2` | `(bit-shift-left x1 x2)` |
| `x1 >> x2` | `(bit-shift-right x1 x2)` |
| `x1 & x2` | `(and x1 x2)` |
| `x1 \| x2` | `(or x1 x2)` |
| `x1 ^ x2` | `(xor x1 x2)` |
| `x1.lt(x2)` | `(< x1 x2)` |
...
| `~x1` | `(not x1)` |
| `-x1` | `(- 0 x1)` |
| `operator.abs(x1)` | `(abs-math x1)` |
| `math.floor(x1)` | `(floor-math x1)` |
| `math.ceil(x1)` | `(ceil-math x1)` |
| `math.trunc(x1)` | `(trunc-math x1)` |
| `builtins.round(x1)` | `(round-math x1)` |
| `x1.eq(x2)` | `(== x1 x2)` |
| `x1.ne(x2)` | `(not (== x1 x2))` |

<!-- end generated atom operators -->

Reflected forms work too: `1 + V.x` builds `(+ 1 $x)`.

The immutable `metta._atoms.operators.OPERATOR_LOWERINGS` table joins Python's
source operations to their MeTTa atom meanings and generates the table above.
A row selects a native symbol, a composite term, a library-provided name, or
a comparison method whose Python operator is reserved for comparing atoms.
`matmul` is provided: `@`
