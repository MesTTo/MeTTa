...
% Guarantees: a singleton superposition adds no return unification and preserves every answer of its member [tested: singleton_superpose:the_only_branch_has_the_same_goal_and_output_as_its_expression, singleton_superpose:the_only_branch_keeps_every_answer_and_duplicate; commit=WORKTREE].
