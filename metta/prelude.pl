...
%   [tested: prelude_spec; commit=WORKTREE].
%
...
prelude_document(documented, ['@doc', documented, ['@desc', "Every name the CURRENT SPACE documents, one per solution; program-scoped on purpose, so the engine's own vocabulary does not bury yours"], ['@params', []], ['@return', "A name"]]).
...
prelude_document('defined-name', ['@doc', 'defined-name', ['@desc', "Every function name the current space defines, builtins excluded, once each"], ['@params', []], ['@return', "A name"]]).
prelude_document(undocumented, ['@doc', undocumented, ['@desc', "Every defined name in the current space with no documentation anywhere; the gap worth closing"], ['@params', []], ['@return', "A name"]]).
