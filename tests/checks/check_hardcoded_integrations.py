...
ALLOWED: dict[tuple[str, str], Site] = {
    ...
    ("python", "pyarrow"): Site(REGISTRANTS, "metta.seam.ipc"),
    ...
    ("python", "graphql"): Site(
        ("extensions/python/metta/_schemas.py",),
        "the reference GraphQL implementation the seat's executor is written "
        "against; the SDL is built as text and needs none",
    ),
    ("python", "opentelemetry"): Site(
        ("extensions/python/metta/telemetry.py",),
        "the OpenTelemetry API, the one standard; SDKs and exporters are the "
        "consumer's and are never imported here",
    ),
