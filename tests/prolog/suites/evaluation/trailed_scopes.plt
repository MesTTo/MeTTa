...
scope(source_recompile, '$metta_source_publication').
scope(source_owner, '$metta_source_publication').
scope(source_load, '$metta_source_publication').
...
within(observation_suspension, Goal) :-
    observation_fixture(
        ( observation_tree(Tree), Context=compiling([],Tree,[]),
          setup_call_cleanup(
              nb_linkval('$metta_observed_runnable',
...
observation_fixture(Goal) :-
    ...
    setup_call_cleanup(
        ( nb_linkval('$metta_observation',Buffer),
          ...
          nb_linkval('$metta_source_context',Context) ),
        Goal,
        forall(member(Key,['$metta_observation','$metta_pending_source_maps',
                           '$metta_source_context']),
               nb_delete(Key))).
...
inside(observation_suspension) :- \+ nb_current('$metta_observed_runnable',_).
...
sweep(Site, Key, Budget, Completed) :-
    ...
    minimum_budget(scope_fixture(Site),
                   (nb_setval(Key,Inactive),once(within(Site,inside(Site)))),
...
sweep_entry(Site, Key, Budget, Result) :-
    ...
    -> true ; Outcome=failed ),
    % Inactive is the trailed doors' [] or the nb_linkval scopes' absence, the
    % suite's own context_inactive/1 reading; a root holding anything else leaked.
    ( context_inactive(Key) -> true
    ...
    context_inactive(Key), operation_clean(Site),
...
test(nested_directory_and_owner_contexts_preserve_enumeration_order) :-
    ...
    metta_with_trailed('$metta_source_publication', source_context([outer], [], [outer], outer),
...
operation(source_rollback, '$metta_source_publication').
...
context_inactive(Key) :- inactive(Key, Inactive), \+ (nb_current(Key,Value), Value \== Inactive).

% What a root reads when no scope holds it. The publication context is one
% term holding the load stack, the recompile stack, the resolved owners and
% the stored-atom owner (engine/filereader/source_lifecycle.pl); every other
% root rests at [].
inactive('$metta_source_publication', source_context([], [], [], none)) :- !.
inactive(_, []).
...
operation_sweep(Site,Key,Budget,Completed) :-
    inactive(Key, Inactive),
    minimum_budget((nb_setval(Key,Inactive),once(perform(Site))),once(perform(Site)),Minimum),
    sweep_trials(operation_entry(Site,Key),Budget,Minimum,Completed).

operation_entry(Site,Key,Budget,Result) :-
    inactive(Key, Inactive),
    nb_setval(Key,Inactive), once(perform(Site)), nb_setval(Key,Inactive),
    ( bounded_call(once(perform(Site)), Budget, Outcome)
    -> true ; throw(error(operation_failed(Site,Budget),none)) ),
    ( nb_current(Key,Inactive) -> true
    ; nb_current(Key,Leaked), throw(error(scope_leaked(Site,Budget,Leaked),none)) ),
    operation_clean(Site),
    once(perform(Site)), nb_current(Key,Inactive), operation_clean(Site),
...
probe_stack_read(Directory) :- filereader:working_dir(Directory).
...
test(a_call_site_carries_the_read_rather_than_a_call) :-
    ...
    Stack = (filereader:nb_current('$metta_working_dirs', [_|_]), _),
...
cost_loop(stack_absent) :- ( between(1, 10000, _), \+ filereader:working_dir(_), fail ; true ).
cost_loop(stack_present) :- ( between(1, 10000, _), filereader:working_dir(_), fail ; true ).
...
test(an_inactive_reader_costs_what_the_asserted_guard_cost,
     [setup((nb_delete('$metta_support_graph_locked'), nb_delete('$metta_working_dirs'),
             retractall(reader_cost_fact(_)))),
      cleanup((nb_setval('$metta_support_graph_locked', []), nb_setval('$metta_working_dirs', []),
               retractall(reader_cost_fact(_))))]) :-
    ...
    nb_setval('$metta_support_graph_locked', []), nb_setval('$metta_working_dirs', []),
    ...
    nb_setval('$metta_support_graph_locked', true), nb_setval('$metta_working_dirs', [guard]),
...
test(a_one_element_stack_reads_without_a_choicepoint,
     [cleanup(nb_setval('$metta_working_dirs', []))]) :-
    nb_setval('$metta_working_dirs', [only]),
    prolog_current_choice(Before),
    filereader:working_dir(Read),
    prolog_current_choice(After),
    assertion(Read == only), assertion(Before == After),
    nb_setval('$metta_working_dirs', [inner, outer]),
    findall(L, filereader:working_dir(L), Both),
