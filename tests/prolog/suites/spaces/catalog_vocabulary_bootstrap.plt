/* Purpose: compare the compiled vocabulary seed, published through the
   tokenized storage funnel, with ordinary publication, and exercise the
   states that require admission or observable row arrival.
...
:- ( predicate_property('$metta_atoms:&metta':'&metta'(_,_,_,_), indexed(Indexes))
   -> true ; Indexes=[] ),
...
cb_rows(Rows, Refs) :-
    findall([Head,Subject,Type]-Ref,
            ( clause('$metta_atoms:&metta':'&metta'(Head,Subject,Type,_), true, Ref),
...
test(the_compiled_physical_rows_equal_ordinary_publication_in_order) :-
    ...
    % Every published row is a native occurrence with a minted token; the
    % payload is the recipe compiled, and the rows are the payload in order.
    forall(member(Ref, Refs),
           ( clause(_:Clause, true, Ref),
             spaces:metta_storage_term(_, _, Token, Clause),
             assertion(integer(Token)) )),
    catalog_vocabulary_seed:seed_payload(Payload),
    catalog_vocabulary_seed:seed_rows(Recipe),
    assertion(Payload == Recipe),
