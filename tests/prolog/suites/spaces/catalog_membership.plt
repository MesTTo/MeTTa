...
test(transaction_erasure_of_a_committed_reference_is_visible) :-
    setup_call_cleanup(
        cm_store([vocabulary, cm_committed, word], Ref),
...
%The storage funnel below the catalog's admission: the ordinary clause with
%its minted token, and nothing else the add door does.
cm_store(Row, Ref) :-
    spaces:ensure_native_storage_module('&metta', Module),
    spaces:add_sexp_in(Module, '&metta', Row, _, Ref).
...
cm_cost_fixture(Size, Words, [Base|Refs]) :-
    ...
    cm_store([vocabulary, cm_size], Base),
    findall(Word-Ref,
            ( between(1, Size, I), atom_concat(word_, I, Word),
              cm_store(['vocabulary-member', cm_size, Word], Ref) ), Pairs),
