...
% This unit's own row among the hook's clauses. The arming point is
% engine/metta/limits.pl, which asserts one clause per registered row into
% `prolog`, so a body arrives wrapped in the module it is asserted into as
% well as the module that declared it; the count strips the wrappers rather
% than pinning one spelling of them.
hook_clauses(Count) :-
    aggregate_all(count,
                  ( clause(prolog:prolog_exception_hook(inference_limit_exceeded, _, _, _, _),
                           Body),
                    hook_goal(Body, spaces:metta_receipt_schedule_reconciliation) ),
                  Count).

hook_goal(Goal, Goal) :- !.
hook_goal(_:Wrapped, Goal) :- hook_goal(Wrapped, Goal).
