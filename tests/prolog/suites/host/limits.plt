...
test(the_limit_hook_is_armed_by_the_first_bound) :-
    ...
    setup_call_cleanup(
        process_create(Swipl,
            ['-f', none, '-q', '-s', File,
             '-g', 'plunit_limits:limit_hook_child', '-t', halt],
...
% The hook's clauses, one per registered seam:bound_hook/2 row, asserted by
% this unit's wrapper at the first bound; before it there are none.
hook_clauses(Count) :-
    aggregate_all(count,
                  clause(prolog:prolog_exception_hook(inference_limit_exceeded, _, _, _, _), _),
                  Count).
