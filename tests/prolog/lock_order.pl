...
%   the workspace root. The four that were one shape, a watched transaction
%   frame finishing inside a critical section delivering frame_finished
%   whose reference refresh needed that section's mutex, went with the
%   listener: reference completion runs from a transaction-exit hook
%   (engine/metta/reference_refresh.pl). The two left put deferred
%   translation on both sides of the typing policy.
:- dynamic known_cycle/2.
known_cycle(['$metta_metta_exec','$metta_typing_policy'], reference_providers).
