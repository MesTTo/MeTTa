...
| `conformance/` | the arbiter, PeTTa, vendored in `petta/` with the runner that gates on it; `critical_pairs_run.pl`, the critical-pair enumerator's own report; and `answer_groups.pl`, which prints one answer group per runnable for any comparator |
