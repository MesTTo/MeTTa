...
% Guarantees: a written `unify` reaches all three fragments and both refusals, because metta_seq_pair_plan/4 parses BOTH operands and classifies over the two parsed sides [tested: tests/prolog/suites/reader/segments.plt:segments_written_pairs; commit=WORKTREE].
