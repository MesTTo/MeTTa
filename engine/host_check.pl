...
%   - metta_require_patched_host/0 succeeds exactly when, beyond that, every
%     required host_patch(File, Sha256) appears in the host's declaration with
%     the same digest, and otherwise throws
%     error(metta_host_unpatched(Missing, Stale, Declaration), _), where Missing
%     lists patches the declaration lacks, Stale those it lists under another
%     digest, and Declaration is the file read or none(Home)
%     [tested: tests/prolog/suites/host/host_check.plt; commit=WORKTREE].
...
metta_require_patched_host :-
    ...
    ;   pairs_keys(Missing0, Missing),
        ...
        throw(error(metta_host_unpatched(Missing, Stale, Declaration), _))
    ).
...
prolog:message(error(metta_host_unpatched(Missing, Stale, Declaration), _)) -->
    { length(Missing, NMissing), length(Stale, NStale),
      aggregate_all(count, metta_host_patches:host_patch(_, _), NRequired) },
    [ 'This SWI-Prolog does not carry every patch the MeTTa engine needs, so the engine does not boot on it.'-[], nl ],
...
declaration_line(none(Home), NRequired) -->
    [ 'The home ~w has no metta-host.pl declaring any of the ~d.'-[Home, NRequired], nl ].
declaration_line(File, NRequired) -->
    { atom(File) },
    [ 'Read from ~w, against the ~d the engine requires.'-[File, NRequired], nl ].
