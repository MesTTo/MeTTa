...
%   execution [tested: function_free_materialization; commit=WORKTREE].
...
% an_older_owned_release_retires_a_concurrently_published_image; commit=WORKTREE]
...
% [tested: a_native_literal_self_does_not_acquire_reader_binding; commit=WORKTREE]
...
% [source: Motik et al., algorithm 1 and section 4; commit=WORKTREE]
derive_ground_bags(GroundRules, Rows) :-
...
% [source: TopologicalSorter.done; commit=WORKTREE]
...
% [source: Impact of transactions, Last modified generation; commit=WORKTREE]
...
% [source: release_trie_ref; commit=WORKTREE]
...
% [tested: a_many_value_call_keeps_constant_prefix_startup; commit=WORKTREE]
