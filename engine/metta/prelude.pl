...
prelude_document('get-property', ['@doc', 'get-property', ['@desc', "The two-argument package surface: (get-property perform claims), a space's package keys and available alternatives, and a package's keys, read at the home a from load made for it or, with none, from its own source"], ['@params', [['@param', "perform, a space, or a package: a library name, a ./, ../ or / path, or (library Name)"], ['@param', "claims, available, or a package key"]]], ['@return', "One property value per answer; a package naming no source, and a computed row or the available key with no home, is refused by name"]]).
prelude_document('setup!', ['@doc', 'setup!', ['@desc', "Prepare a package and its requirements explicitly. Reuse setup only while its performed rows and backing artifacts remain current; publish performed.metta and the transitive lock.metta under an OS directory lock"], ['@params', [['@param', "A package: a library name, a ./, ../ or / path, or (library Name)"]]], ['@return', "Bool"]]).
prelude_document('package-load', ['@doc', 'package-load', ['@desc', "A required packaging library defines this equation to replace the engine's default package interpreter. Arguments are the canonical source path, home space and the source's own package rows; requires have already loaded under the engine's ceiling and budget"], ['@params', [['@param', "Canonical path"], ['@param', "Home space"], ['@param', "Package argument rows"]]], ['@return', "Any successful result"]]).
prelude_document('package-prolog', ['@doc', 'package-prolog', ['@desc', "The claimant the engine registers for a (prolog <file> (heads)) backing row. A seat claims its own token the same way, by adding a (= (perform (<token> ...)) ...) equation to the seam space"], ['@params', [['@param', "Artifact the row names"], ['@param', "Heads the row claims"]]], ['@return', "Bool"]]).
