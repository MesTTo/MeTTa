...
:- use_module('owned_resources', [with_outcome_cleanup/3]).
...
metta_engine:builtin_implementation('get-property'/2, prolog(packages)).
...
package_validate_boot(Space, Row) :-
    ...
    Row = [Token|Arguments],
    findall(Type,
            % policy-inventory-exempt: mechanism-internal; reason=the two places a boot row's declaration can be written, the file's own space and the seam where claims live; evidence=engine/packages.pl:package_validate_boot/2
    ...
    ; member(Type, Types), metta_engine:metta_arrow_type_chain(Type, Chain),
      ...
      metta_engine:with_metta_module(Module,
          packages:maplist(package_argument_type, Arguments, Expected))
    -> true
...
% [tested: package_laws:default_claim_recovers_after_withdrawal_and_failed_activation; commit=561cfeaa23b27fc84f86a9bcccf6ccf8b9d2e73f].
package_register_claims :-
    ( package_default_claim -> true
    ; with_mutex(package_claims,
        ( package_default_claim -> true
        ; filereader:with_owning_source_load(none, packages:
...
package_install_hooks :-
    ...
    ; with_mutex(package_claims,
        ...
        ; wrap_predicate(metta_engine:metta_unimport(Space, Path), package_lifetime,
                         Wrapped, packages:package_unimport(Space, Path, Wrapped)),
          wrap_predicate(metta_engine:metta_reference_check_manifest(Home, File, Forms, Seen0, Seen),
                         package_admission, Manifest,
                         packages:package_manifest(Home, File, Forms, Seen0, Seen, Manifest)),
          at_halt(packages:package_close_all),
...
package_catalog_row([package, Name, Where]) :-
    ...
    % policy-inventory-exempt: mechanism-internal; reason=POSIX's own two directory entries, which directory_files/2 always returns and no operator chooses; evidence=engine/packages.pl:package_catalog_row/1
    \+ memberchk(Name, ['.', '..']),
