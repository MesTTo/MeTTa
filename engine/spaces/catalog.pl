...
% Guarantees: metta_publish_every_vocabulary_type/0 publishes the compiled
...
metta_publish_every_vocabulary_type :-
    % The seed's payload predicate, not the module: naming the module in a
    % clause body below creates it when this file loads.
    \+ current_predicate(catalog_vocabulary_seed:seed_payload/1),
    ...
    forall(( member(Head, [':', ':<']),
             metta_catalog_row([Head, Subject, _]) ),
    ...
    (   metta_catalog_row([':', IndexedSubject, _])
    ->  native_storage_module('&metta', Storage),
        metta_storage_term('&metta', [':', IndexedSubject, _], _, Indexed),
        once(clause(Storage:Indexed, true))
    ;   true
    ).
metta_publish_every_vocabulary_type :-
    (   metta_vocabulary_types_published
...
metta_vocabulary_seed_context :-
    ...
    % policy-inventory-exempt: mechanism-internal; reason=the four catalog heads that describe a vocabulary, its members, its type name and its order, which the seed's recipe reads and nothing else may have changed; evidence=engine/spaces/catalog.pl:metta_catalog_head/1
