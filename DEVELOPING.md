...
### The four report lanes over the suite itself

None of these gates. Each prints a number the GATE lanes cannot see.
...
- `stubtest` compares the shipped `.pyi` files against the runtime objects they
  describe. Its allowlist is
  `extensions/python/tests/data/stubtest-allowlist.txt`, each entry under the
  reason it is there, and its mypy settings are
  `extensions/python/tests/data/stubtest-mypy.toml`, which are stubtest's own
  and not the project's type policy.
