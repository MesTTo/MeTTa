...
#
#   Usage: sh tools/check.sh [--component path] [--list] [name ...]     names: ruff mypy ty pylint perflint
...
# A seat delegates runtime setup here; ownership restricts which registrations run.
CHECK_COMPONENT=
if [ "${1:-}" = --component ]; then
    [ "$#" -ge 2 ] || { echo "check.sh: --component needs a component path" >&2; exit 2; }
    CHECK_COMPONENT=$2
    shift 2
    case "$CHECK_COMPONENT" in
        ext|extensions/*) ;;
        *) echo "check.sh: invalid component: $CHECK_COMPONENT" >&2; exit 2 ;;
    esac
    case "$CHECK_COMPONENT" in
        *..*) echo "check.sh: invalid component: $CHECK_COMPONENT" >&2; exit 2 ;;
    esac
    [ -f "$HERE/$CHECK_COMPONENT/check.sh" ] || {
        echo "check.sh: component has no check.sh: $CHECK_COMPONENT" >&2; exit 2;
    }
fi
CHECK_OWNER=
CHECK_LIST=0
...
GENERATED_ARTIFACT_LANES="layer-sync protocol-sync vocab-sync artifact-sync binding bounds-sync codec-doc example-origins face-sync prolog-face pygments-sync refusal-sync aio-mirror fn-sync libdoc refusals init-stub door-sync ledger phrasebook reference"
WANT="$*"
case " $WANT " in
...
run() {
    ...
    if [ -n "$CHECK_COMPONENT" ] && [ "$CHECK_COMPONENT" != "$CHECK_OWNER" ]; then
        return 0
    fi
    if [ -n "$WANT" ]; then
        case " $WANT " in *" $name "*) ;; *) return 0 ;; esac
    fi
    MATCHED_LANES="$MATCHED_LANES $name"
...
# Umbrella: the execution-plan anchors span engine, libraries, seats and workspace gates.
...
# Umbrella: engine policy rows must agree with consuming Prolog and Python seams.
...
# vacuously.
# Umbrella: Python semantic fences, engine segment fences and the upstream corpus supply refusal grounds.
...
# Umbrella: library contracts are witnessed across the shared Prolog suites.
...
# Umbrella: superproject gitlinks must resolve in every component remote.
...
# cannot pass.
# Umbrella: documentation across components links into the shared example corpus.
...
# Umbrella: C, Node and Prolog loaders must follow the engine artifact-purge contract.
...
# Umbrella: compiled artifacts under both engine/ and lib/ must name this workspace.
...
# Umbrella: the upstream corpus compares the assembled engine, libraries and extensions.
...
# Umbrella: generated corpus programs compare upstream with the assembled engine and libraries.
...
# Umbrella: workspace and component cheat sheets name the engine, libraries and seat APIs.
...
# Umbrella: workspace checker fixtures and the Python artifact generator must agree on driver paths.
run GATE gate-layout-selftest "$PY" "$HERE/tests/checks/check_gate_layout_selftest.py"
# Umbrella: shared checker fixtures import modules from workspace and seat paths.
...
# Umbrella: the workspace KERNEL.md ledger must agree with the running engine translator.
...
# Umbrella: tracked sources in the superproject and all component repositories are scanned.
...
# Umbrella: the superproject index must match populated component checkouts.
...
# Umbrella: library package rows, the engine importer and Python package reader must agree.
...
# Umbrella: the shared publisher validates release artifacts from Python and ext distributions.
...
# `pymetta_host-*`.
# Umbrella: Python and host/integration artifacts must match the workflow publisher partition.
...
# remembers to run.
# Umbrella: mutations check the Python/ext partition against both workflow upload jobs.
...
# Umbrella: front pages across component repositories must share the same introduction.
...
# Umbrella: seat consumers must resolve build artifacts from the engine and chapter-19 examples.
...
# Umbrella: Python, Node and C seats must keep integration names behind registration.
...
# Umbrella: Python core and ext distributions must respect the workspace import boundary.
...
# Umbrella: engine, seat and workspace runners must all enforce the shared child-lifetime contract.
...
# Umbrella: the shared wrapper must reap processes started by engine, seat and workspace runners.
...
# Umbrella: artifact inputs and outputs span examples, extensions/python.
...
artifact_example_origins_witnesses() {
    ...
    bounded "$PY" "$HERE/tests/checks/check_library_records_selftest.py" || return $?
}
# Umbrella: artifact inputs and outputs span examples, extensions/python.
run GATE example-origins-selftest artifact_example_origins_witnesses

# Umbrella: artifact inputs and outputs span ext, extensions/python, lib.
...
# Umbrella: artifact inputs and outputs span extensions/python, lib, tests.
run GATE prolog-face "$PY" "$HERE/extensions/python/tools/prologface.py"
artifact_prolog_face_witnesses() {
    bounded "$PY" "$HERE/tests/checks/check_prologface_selftest.py" || return $?
}
# Umbrella: artifact inputs and outputs span extensions/python, lib, tests.
run GATE prolog-face-selftest artifact_prolog_face_witnesses

# Umbrella: artifact inputs and outputs span extensions/python, website.
...
# Umbrella: artifact inputs and outputs span engine, extensions/python, tests.
...
# Umbrella: artifact inputs and outputs span ext, extensions/python.
...
# Umbrella: artifact inputs and outputs span lib, llms.txt, website.
...
# Umbrella: artifact inputs and outputs span engine, tests, website.
...
# Umbrella: artifact inputs and outputs span ext, extensions/python, llms.txt, website.
...
# Umbrella: artifact inputs and outputs span extensions/python, lib, website.
...
# Umbrella: Python core doors and ext registrants share the generated door catalog.
run GATE door-coverage "$PY" "$HERE/extensions/python/tools/doorgen.py" --coverage
# Umbrella: Python core doors and ext registrants share executable refusal witnesses.
run GATE door-refusals "$PY" "$HERE/extensions/python/tools/doorgen.py" --refusals
# Umbrella: engine, seat and workspace lanes share concurrent gate scratch allocation.
run GATE scratch-retention "$PY" "$HERE/tests/checks/check_gate_scratch_selftest.py"
# Umbrella: the Python lexer and website TextMate grammar must agree over a shared corpus.
...
# Umbrella: Python core doors and ext registrants are analyzed together for dependency order.
...
# Umbrella: website tutorial fences must match Python, library and example sources.
...
# Umbrella: the external Jupyter kernel must run against both upstream PeTTa and this engine's host bridge.
...
# Umbrella: Python drivers across engine, seats and workspace tooling share the root lint configuration.
...
# Umbrella: the website builds shared engine contracts, contributor documentation and seat references.
...
# loaded database.
# Umbrella: duplicate Prolog spans engine, libraries, seat bridges and shared suites.
...
# Umbrella: llms.txt must describe Python and ext exports, library heads and shared entry points.
run REPORT llms-coverage "$PY" "$HERE/tests/checks/check_llms_coverage.py"
