...
- FROM source origins: a declaration-only `(from &home (rename ((Canonical Alias))))`
  ...
  match the provider's values and `get-type` answers the canonical type. Explicit
  bindings, supplied Atom values and `bind!` token values keep their literal heads;
  a local equation head or declaration subject shadows the imported name within its
  form; later source follows the executed prefix, so a future equation does not
  shadow an earlier use. Distinct canonical heads under one alias refuse only when
  that alias is used (`metta_source_name_ambiguity`), a mapper that needs its own
  unfinished map refuses (`metta_source_mapping_cycle`), and withdrawing the FROM
  row restores literal source while retained values and compiled equations keep
  their resolved heads. Native suite `reference_source_origins`, Python
  `test_reference_source_origins.py`.
...
- Python program conversion emits native MeTTa that reconstructs referenced
  class spaces, shared spaces, cycles, lexical bindings and translator rules.
  Direct source views omit metadata regenerated by references. Source-owned
  token claims and registrations roll back with failed loads, and owned spaces
...
- `current_predicate_wrapper/4` keeps every other retained closure's identity
...
- A class definition publishes its references once: a Python transaction is a
  definition batch, so reference publication and dependent recompilation wait
  for the first evaluating door inside it or for its end, as a file's wait for
  its next runnable; a provider's face is remembered by home and blocked path
  set while the rows stand instead of being recomputed for every importer and
  every row; and a repair filed for a module released before the batch's drain
  is dropped. The four-class method diamond of the class methods tests costs
  13.8M inferences to define where it cost 257.6M.
...
- `seam:form_rewriter/1` callables take four arguments, `Term, Origins,
  ...
  transforms both; a malformed tree raises `metta_source_origins`. The reader
  entry is `rewrite_parsed_form/5` with `origin(Kind, Origins)`. Source
  observation aligns a rewriter's output with the written form position by
  position: subtrees returned unchanged keep their coordinates, every node above
  a change is reported as generated by `form-rewriter` at the whole form, and the
  span it displaced reports its coverage unavailable.
...
- The translator loaded on its own, as the Python shim's `use_module` loads it
  ...
  does, so `engine/translator/runtime.pl`'s directive no longer fails with
  `metta_engine_src_dir/1` unknown and the plunit lane's two host suites load
  without an error.
...
- Effect plans follow renamed and patterned references to their defining
  ...
  lexical meaning. Operation reclassification and reference withdrawal are
  visible to the next inspection without executing the inspected program.
...
- The Python seam gains a fourteenth point, `ipc`, beside `arrow`: who writes
  and reads the Arrow IPC streaming format (`claims`, `schema`, `stream`,
  `read`, `concat`), with pyarrow as the shipped row. The gateway's Arrow
  answers and the remote cursor's `to_arrow()` reach the encoder through it,
  so a second encoder is a row from its own package and no edit here; the
  `no-hardcoded-integration` lane found the direct import the projections
  work had made, and the point is the repair. graphql-core and the
  OpenTelemetry API are declared as this seat's own dependencies in that
  lane's table, each with the reason.
...
- The C seat has the seam and three capabilities it did not have. A library
  ...
  (`mt_provider_open`, which rides the engine's foreign-space seam, speaks
  canonical MeTTa text, enumerates by index, and takes the name at the
  engine's claim door so one another provider owns is refused by name), say
  how its own object type prints (`mt_repr`), and register a directory of
  MeTTa or Prolog sources (`mt_library`) -- and be LOADED at all, by
  `mt_extension(m, path)`, which calls the `mt_extension_init` the library
  exports, sqlite3's loadable-extension shape entry point and all.
  `mt_point_declare`, `mt_register`, `mt_unregister`, `mt_point_count`,
  `mt_point_at`, `mt_point_of`, `mt_seam_count`, `mt_seam_at` and `mt_claim`
...
- The twins band's authoring allowance is re-measured. It read 1456 inferences
  ...
  Re-measured again on the tree that merged the lane's burn-down, after the
  catalog-types merge began checking a definition's delivery word at the
  write: 1406 once plus 1309 each.

- A capability-gated twin declares `available(m)`. Asked by the twins lane
  before `twin(m)` and outside the counted window, it says whether the
  capability the twin and its example guard on (a redis server, the MORK
  artifact, an importable torch) is present here; a `BUDGET` measured where it
  was present is compared only where it answers True, and the lane says
  "budget not compared here" where it answers False instead of reading a
  51,246-inference fall as the tree's. The selftest plants both answers.
...
- **Every refusal kind is a catalog row, and every seat reads it.** The
  `&metta` catalog holds one `(refusal <kind> <class> (ground <authority>
  "<citation>") (remedy "<title>" <kind> <applicability> <act>...))` row per
  refusal the engine raises on purpose, thirteen of them, declared in
  `engine/spaces/catalog.pl` beside the kind table they are held equal to.
  `metta_host_refusal/6` reads a raised ball's row and fills the remedy's
  `<field>` holes from that ball, so the repair is rendered ONCE, by the
  engine, and every seat carries the same sentence: `.ground` and `.remedy` on
  every engine refusal in Python, the same two on `MettaError` in TypeScript
  (and in its `toJSON`), and `mt_ground()` and `mt_remedy()` beside
  `mt_errmsg()` in C. A hole the ball cannot fill is the reader's own choice
  and keeps the remedy `prose`, which is rustc's `HasPlaceholders`; a remedy
  whose holes were all filled keeps its row's applicability, so a capability
  refusal arrives `maybe` carrying `(edit (grants &restricted process))`.

  A `prose` `Remedy` may now be its title alone. Nine of the thirteen kinds
  have no mechanical repair, because their repair is a decision, and advice
  with no edit is what PostgreSQL's `errhint()` and clang's `note:` carry. A
  `machine` or `maybe` remedy naming no act is still a construction error.

  `website/reference/refusals.md` is those rows rendered, generated by
  `extensions/python/tools/refusalsdoc.py` and gated by the `refusals` lane;
  the site had no error page at all before it. `refusal-kind`, `ground-kind`,
  `remedy-kind` and `applicability` are catalog vocabularies now, so
  `metta.vocabularies` and `@metta/node`'s `vocabularies.ts` carry
  `RefusalKind`, `GroundKind`, `RemedyKind` and `Applicability`, and the Node
  seat's hand-written kind union is the generated one.
  `tests/data/error-kinds.json` records what each seat spells instead of the
  row's class and WHY, so a difference between the seats is a decision rather
  than a drift.
